...
final class FetchMealTests: XCTestCase {
    func testGetIngredientIndex() {
        ...
        let oneDigitIndex = getIngredientIndex(from: ingredientLabel)
        let twoDigitIndex = getIngredientIndex(from: measureLabel)
        let invalidIndex = getIngredientIndex(from: invalidLabel)
    ...
    func testSuccessfulFormatMealDetail() {
        ...
        let output = formatMealDetail(from: mealDetail)
